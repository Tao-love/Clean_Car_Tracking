...
/*
 * MG310 左右 AB 相编码器模块。
 * 职责：A 相双边沿中断结合 B 相判向，并每 6.667 ms 计算原始/平滑速度。
 * 依赖：SysConfig GPIO_ENCODER。上下文：GROUP1 ISR 只更新计数，速度更新在主循环 150 Hz 任务。
 * 单位：count 是 A 相双边沿计数，speed 是每 6.667 ms 计数。硬件副作用：开启 GROUP1 NVIC。
...
/* 用途：快照计数并计算一次 6.667 ms 速度与 0.3/0.7 平滑值；禁止 ISR 调用。 */
