...
/*
 * 本地安全联锁与统一停车模块。
 * 职责：在 MCU 本地以 10 ms 粒度判定心跳超时、丢线、左右堵转和控制超期，并执行唯一停车路径。
 * 依赖：motor、ControlParams、ControlSample。上下文：100 Hz 主循环任务。
 * 单位：所有持续时间用 10 ms tick。硬件副作用：故障时立即清 PWM 和四方向脚。
...
#define SAFETY_COMM_TIMEOUT_TICKS (40U)
#define SAFETY_LINE_LOST_TICKS    (15U)
#define SAFETY_STALL_TICKS        (30U)
...
/* 用途：在 trial 开始前清计时器并记录最近合法帧 tick；不清除未确认的故障。 */
