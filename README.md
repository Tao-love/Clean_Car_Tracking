...
- 蓝牙启动的 RUNNING 试验中，400 ms 无合法帧、150 ms 丢线、300 ms 堵转或连续控制超期均由 MCU 本地停车。
...
- KEY1 是 `PA23` 上拉输入；当前按下沿会在 `IDLE` 请求一次 `LINE_FOLLOW` 的 5 秒试验，按住不会重复启动。
- KEY1 启动的试验仅跳过“400 ms 未收到蓝牙合法帧”的通信超时；丢线、堵转、控制超期、参数校验和 5 秒自动停止全部保留。
- 蓝牙启动仍要求心跳。蓝牙恢复后，`ABORT_TRIAL` 可提前停止蓝牙或 KEY1 启动的试验。
- KEY1 当前不清除 `FAULT`；故障必须先恢复安全条件，再通过现有蓝牙 `CLEAR_FAULT` 清除。

完整的当前工程补充见 [docs/specs/1-3.md](docs/specs/1-3.md)。
