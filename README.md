...
UART1 使用 `9600 8N1`，并执行原样字节回显，测试引脚如下：
...
只连接 TX、RX 和 GND；**不要将 USB-TTL 的 3.3V 或 5V 接到板上。** 小车由自身电池正常供电后，再连接 UART 信号线。向 UART1 发送任意字节序列会收到完全相同的字节序列；该临时模式不处理既有二进制协议帧。
