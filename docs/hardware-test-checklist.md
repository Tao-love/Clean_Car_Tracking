...
本清单保留给以后烧录、接车和赛道联调时使用。用户已明确本次交付**不要求烧录或实车检验**，所以下列项目不属于当前“1-3 软件实现完成”的验收条件，也不阻塞代码、离线测试、编译链接和 GitHub 交付。

以后决定进行实物验收时，应按下列顺序执行。当前未由实物验证的项目统一标为 `WAITING_FOR_HARDWARE_TEST`；这表示“留待以后验证”，不表示软件功能尚未实现，也不能被误写为已经实测通过。
